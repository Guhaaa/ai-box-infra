...
шагам в секции «Миграция данных» дизайн-спеки.
...
Домены копии задаются в `.env` (`FRONT_DOMAIN`/`API_DOMAIN`/`ADMIN_DOMAIN`,
опционально `CERT_NAME`) — публичные vhost'ы рендерятся из
`nginx/templates/*.template` при старте контейнера nginx.
