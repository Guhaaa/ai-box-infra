...
Qdrant) сняты до мержа. Плоский `.env` на doitai **удалён в тот же день** — переход на стенде
завершён полностью (ключи сверены по значениям, копии вне git, после удаления
перепроверены пути деплоя/cron/reload). Стенд **amulex** сознательно не трогаем
(решение 2026-07-30) — остаётся на плоском `.env`. Боевые уроки — `docs/runbooks/env-per-stend-migration.md`.
