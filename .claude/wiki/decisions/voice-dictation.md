...
## Что осталось на боевую раскатку (по runbook, с разрешения человека)

- проверка достижимости ASR-хоста из nginx-контейнера
  (`docker compose exec nginx nc -vz <asr> <port>`) — с прод-сервера;
- прописать реальный `ASR_WS_UPSTREAM` в `.env` doitai;
- `nginx -s reload`, полный поток при `asr.enabled=true` (питон-зонд из брифа);
- (опц.) firewall на ASR-хосте.

## Проверено локально

`nginx -t` чист на обоих шаблонах (прод-шаблон — полным рендером, тест —
изолированным рендером). Закрытый гейт → **403** на обеих поверхностях
